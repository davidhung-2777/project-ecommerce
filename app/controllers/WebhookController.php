...
class WebhookController extends Controller
{
    ...
    public function sepay(): void
    {
        ...
        if ($webhookSecret !== '' && !str_starts_with($webhookSecret, 'your_')) {
            if (!$this->sepayService->verifyWebhookSignature($rawBody, $signature, $timestamp)) {
        ...
        if (!$orderCode) {
            ...
            $this->jsonResponse([
                'success' => true,
                'message' => 'Webhook received; no matching order code to process'
            ], 200);
            return;
        }
    ...
    private function extractOrderCode(string $description): ?string
    {
        ...
        // Order numbers generated by this application use the DN prefix.
        if (preg_match('/\b(DN\d+[A-Z0-9]*)\b/', $description, $matches)) {
            return $matches[1];
        }

        // Keep compatibility with older order numbers using the DH prefix.
        if (preg_match('/\b(DH\d+)\b/', $description, $matches)) {
            return $matches[1];
        }
